...
/** Detail page poster: prefer scrape meta, then list poster, then derived frame. */
export function mediaDetailPosterSrc(
  ...
  const encrypted = Boolean(detail.encrypted_asset) || (detail.file_path || "").toLowerCase().endsWith(".enc");
  const fromList = normalizeListPosterUrl(detail.poster_url || "");
  if (fromList) return withAccessToken(fromList);
  if (encrypted) return derivedVideoPosterSrc(detail.id);
  return derivedVideoPosterSrc(detail.id) || localPosterSrc(detail.id, false);
}
