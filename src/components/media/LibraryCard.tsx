...
export default function LibraryCard({ library, onPress, preferredFocus }: Props) {
  const [c1, c2, c3] = libGradient(library.type, library.id);
  const preview = library.preview_url ? absoluteUrl(library.preview_url) : null;

  return (
    <FocusablePressable onPress={onPress} preferredFocus={preferredFocus} style={styles.card} focusedStyle={styles.focused}>
